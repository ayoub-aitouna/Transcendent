		...
		<div className='mt-2 w-full h-[69px] flex items-center justify-between rounded bg-[#292929] p-4 mb-3'>
			<Link
				...
				<Image
					...
					src={ImageSrc(user.image_url, user.username)}
					alt='Profile Image'
					width={53}
					height={53}
				/>
				...
				{!isSearch ? (
					clicked ? (
						<button className='flex flex-row items-center justify-center rounded-[4px] bg-[#474747] w-[180px] h-[35px]'>
							...
							<div className='text-[#e2e2e2] text-[12px] font-medium'>
								Request sent
							</div>
						</button>
					) : (
...
export function PendingContainer({ user }: { user: FriendRequest }) {
			...
			</Link>
			{
				<div className={`flex items-center justify-center rounded-[4px] gap-2`}>
					{accept || decline ? (
						...
						<>
							<button
								className={`flex flex-row items-center justify-center rounded-[4px] bg-[#FD4106] w-[100px] h-[35px] p-2`}
							...
							<button
								className={`flex flex-row items-center justify-center rounded-[4px] bg-[#474747] w-[80px] h-[35px]`}
	...
	return (
		<button
			...
			{clicked ? (
				<Link
					href={"/home"}
					className=' flex items-end justify-end  text-white text-[16px] font-medium'>
					{" "}
					<RightArrow />{" "}
				</Link>
			) : (
				<div
					className={`flex  items-center justify-center rounded-[4px]  bg-[#FD4106] w-[108px] h-[27px]`}
					onClick={() => handleClicked()}>
					<div className=' flex items-center   justify-between mx-auto text-white text-[12px] font-medium'>
						{" "}
						Unblock{" "}
					</div>
				</div>
