...
function NotificationMenu({ id }: { id: number }) {
	...
	const seenNotifications = async () => {
		try {
			...
			addToast({
				id: id,
				title: "Error",
				message: "we have encountered an error while trying to set this Notification as seen",
				icon: "/assets/icons/light_close.png",
				backgroundColor: 'bg-red-500'
			})
		}
	...
	useEffect(() => {
		if (deleteSuccess || seenSuccess) {
			router.refresh()
		}
	}, [deleteSuccess, seenSuccess, router])
		...
		<div className="">
			...
			{clickedThreePoints && (
				<div
					...
					className=" bg-[#161616] z-50 absolute left-[100%] bottom-[-80%] h-[70px] w-[120px] p-4 flex flex-col items-center justify-center rounded-md"
				>
					...
					<button className="flex flex-row items-start justify-start text-[16px] text-[#878787] gap-2 hover:bg-[#262626] py-2 px-2 w-full rounded-md"
						...
						<p className="font-light text-[12px]"> set as seen </p>
					</button>
				</div>
			)}
		</div>
