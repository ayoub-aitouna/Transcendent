...
class UserDetailSerializer(serializers.ModelSerializer):
    ...
    def get_friendship_list(self):
        ...
        friends_as_requester = User.objects.filter(
            Q(requster__addressee=current_user) & Q(requster__is_accepted=True))
...
class FriendRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['image_url', 'fullname',
                  'username', 'url', 'accept_fiend_request']
...
class UserFriedndsSerializer(serializers.ModelSerializer):
    ...
    def get_fiendship_id(self, obj):
        user_id = self.context.get('view').kwargs.get('pk')
        qlist = obj.addressee.filter(Q(requster_id=user_id) | Q(
            addressee_id=user_id)) | obj.requster.filter(Q(requster_id=user_id) | Q(addressee_id=user_id))
